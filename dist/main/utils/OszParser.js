...
class OszParser {
    ...
    async parseOszFile(oszPath, outputDir) {
        try {
            logger_1.logger.debug('osz-parser', `Parsing OSZ file: ${oszPath}`);
            // Create output directory
            await fs_1.promises.mkdir(outputDir, { recursive: true });
            return new Promise((resolve, reject) => {
                yauzl.open(oszPath, { lazyEntries: true }, (err, zipfile) => {
                    if (err) {
                        logger_1.logger.error('osz-parser', `Failed to open OSZ: ${err}`);
                        reject(err);
                        return;
                    }
                    if (!zipfile) {
                    ...
                    zipfile.on('entry', (entry) => {
                        ...
                        const outputPath = (0, path_1.join)(outputDir, (0, path_1.basename)(fileName));
                        zipfile.openReadStream(entry, (streamErr, readStream) => {
                            if (streamErr) {
                                logger_1.logger.error('osz-parser', `Error reading ${fileName}: ${streamErr}`);
                                zipfile.readEntry();
                                return;
                            }
                            if (!readStream) {
                    ...
                    zipfile.on('end', async () => {
                        try {
                            ...
                            if (!songData.title || !songData.artist) {
                                logger_1.logger.warn('osz-parser', 'Missing required song metadata');
                                resolve(null);
                                return;
                            }
                            const finalSongData = {
                            ...
                            logger_1.logger.error('osz-parser', `Error finalizing song data: ${parseError}`);
                            resolve(null);
                        }
                    });
                    zipfile.on('error', (zipErr) => {
                        logger_1.logger.error('osz-parser', `ZIP file error: ${zipErr}`);
                        reject(zipErr);
                    });
                });
            });
            ...
            logger_1.logger.error('osz-parser', `OSZ parsing failed: ${error}`);
            return null;
        }
    }
    /**
     ...
     */
    generateId(filePath) {
        const fileName = (0, path_1.basename)(filePath, '.osz');
...
exports.OszParser = OszParser;
exports.default = OszParser;

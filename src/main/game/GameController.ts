// src/main/game/GameController.ts
import { PinChart, Judgment, Note } from '../../shared/types';
import { windowsFromOD, preemptFromAR } from '../../shared/timing';
...
export class GameController {
  ...
  private chart: PinChart;
  private notes: Note[];
  private startTime: number = 0;
  private isRunning: boolean = false;
  private highResTimer: any;
  private windowsSec = { KOOL: 0.05, COOL: 0.1, GOOD: 0.2, MISS: 0.3 };
  private approachTimeSec = 1.2;
  ...
  private constructor(chart: PinChart) {
    ...
    const { w300, w100, w50 } = windowsFromOD(OD);
    this.windowsSec = {
      KOOL: w300 / 1000,
      COOL: w100 / 1000,
      GOOD: w50 / 1000,
    ...
    const preemptMs = preemptFromAR(AR);
    this.approachTimeSec = preemptMs / 1000;
  }

  public static startGame(chart: PinChart): { success: boolean; error?: string } {
    try {
  ...
  public static stopGame(): { success: boolean; error?: string } {
    try {
      if (GameController.instance) {
        GameController.instance.stop();
      }
  ...
  public static getInstance(): GameController | null {
  ...
  private startGameLoop() {
    ...
    const createHighResTimer = (callback: () => void, intervalMs: number) => {
  ...
  private updateScore(judgment: Judgment, hitErrorMs: number = 0, noteTime?: number) {
    ScoreManager.addScore(SCORE_MAP[judgment]);
    if (judgment !== 'MISS') {
  ...
  public handlePinPress(currentTimeSec?: number) {
    ...
    if (judgment !== 'MISS') {
      ...
      NoteManager.updateNote(closestNote);
    }
